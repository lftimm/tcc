program Post_VTK
    ...
    call get_command_argument(1,pname)
    call get_command_argument(2,iter)
    ...
    ALLOCATE (xyz(nnodes,3), P(nnodes), uvw(nnodes,3),uvwm(nnodes,3),Pm(nnodes))
    ...
    ! Leitura das velocidades medias
    filename(7:14) = ".vm   "
    OPEN(1, file=filename, status="old")
    do I = 1, nnodes
          READ(1,*) uvwm(I,1), uvwm(I,2), uvwm(I,3)
    end do
    CLOSE(1)

    ! Leitura das pressoes medias
    filename(7:14) = ".pm   "
    OPEN(1, file=filename, status="old")
    do I = 1, nnodes
        READ(1,*) Pm(I)
    ...
    WRITE(1,*) "SCALARS PressãoMédia float 1"
    WRITE(1,*) "LOOKUP_TABLE default"
    do I = 1, nnodes
        WRITE(1,21) Pm(I)
    end do
    ...
    WRITE(1,*) "VECTORS VelocidadeMédia float"
    do I = 1, nnodes
        WRITE(1,23) uvwm(I,1), uvwm(I,2), uvwm(I,3)
    end do
    WRITE(1,*) ""
